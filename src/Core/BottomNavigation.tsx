...
export const BottomNavigation: React.FC<bottomNavigationProps> = () => {
    ...
    const renderButton = () => {
        return buttons.map((button) => {
            return (
                    <TouchableOpacity key={button.name} style={[STYLES.Buttons.bottomNavigationButton]} onPress={onNavigationButtonPress(button.name, button.props)}>
                        <View>
        ...
        return (
            <View style={[STYLES.ScreenStyles.BottomNavigationContainer]}>
                {renderButton()}
            </View>
        )
}